...
def test_write_guard_status_warn_without_heartbeat(monkeypatch, tmp_path):
    ...
    assert st["hook_hint_only"] is True
    ...
    # Hint present still ≠ enforcement
    assert st2["severity"] == "warn"
    assert st2["hook_hint_only"] is True
    assert st2["wired_hint"] is True
    assert st2["proven"] is False

    monkeypatch.setenv("GODKILLER_WRITE_GUARD_PROVEN", "1")
    st3 = wg.write_guard_host_status()
    assert st3["severity"] == "ok"
    assert st3["hook_hint_only"] is False
    assert st3["proven"] is True
...
def test_build_honesty_status_shape():
    ...
    assert "hook_hint_only" in st["write_guard"]
    assert "workspace" in st
    assert "write-guard hint ok" not in st["mouth"].lower()
