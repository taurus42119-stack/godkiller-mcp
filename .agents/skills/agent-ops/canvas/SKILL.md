...
**Component discovery:** prefer host canvas components when available over hand-rolled markup. The full public surface (components, hooks, prop types, tokens) is declared in `.agents/skills/agent-ops/canvas/sdk/index.d.ts` and its sibling `.d.ts` files — read them when you need exact exports, prop shapes, or hook signatures rather than guessing. Referencing an export that does not exist is the most common runtime error.
